...
public class TossPaymentExternalRepository {
    ...
    public TossPaymentResponse requestPayment(BigDecimal amount, String customerName) {
        ...
        try {
            ResponseEntity<TossPaymentResponse> response =
                    restTemplate.exchange(TOSS_API_URL, HttpMethod.POST, requestEntity, TossPaymentResponse.class);
    ...
    public TossPaymentResponse confirmPayment(String paymentKey, String orderId, BigDecimal amount) {
        ...
        try {
            ResponseEntity<TossPaymentResponse> response =
                    restTemplate.exchange(TOSS_CONFIRM_URL, HttpMethod.POST, requestEntity, TossPaymentResponse.class);
            return response.getBody();
        } catch (HttpStatusCodeException e) {
            handlePaymentError(e);
            return null;
    ...
    public TossPaymentResponse refundPayment(String paymentKey, BigDecimal originalAmount, BigDecimal refundAmount) {
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        headers.setBasicAuth(secretKey, "");
        ...
        try {
            ResponseEntity<TossPaymentResponse> response =
                    restTemplate.exchange(TOSS_REFUND_URL, HttpMethod.POST, requestEntity, TossPaymentResponse.class);
            return response.getBody();
        } catch (HttpStatusCodeException e) {
            handlePaymentError(e);
            return null;
    ...
    private void handlePaymentError(HttpStatusCodeException e) {
        try {
            ...
            throw new RuntimeException("결제 오류: " + errorResponse.getMessage() + " (코드: " + errorResponse.getCode() + ")");
        } catch (Exception ex) {
