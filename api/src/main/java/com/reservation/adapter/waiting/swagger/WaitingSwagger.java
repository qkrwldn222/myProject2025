...
public interface WaitingSwagger {
  ...
  @PostMapping("/join")
  ResponseEntity<ApiResponseWrapper<Void>> joinQueue(
  ...
  @Operation(summary = "첫번째 대기열 입장 완료", description = "레스토랑 운영자가 첫번째 대기열을 지웁니다.")
  @ApiResponses(
      value = {
        @ApiResponse(responseCode = "200", description = "다음 사용자가 입장 허용됨"),
        @ApiResponse(responseCode = "403", description = "운영자 권한 없음"),
  ...
  @PostMapping("/enter")
  ResponseEntity<ApiResponseWrapper<Void>> enterNextUser(
      @RequestParam("restaurantId") Long restaurantId);
