...
public class MenuRestService implements MenuService {
    ...
    public List<Menu> findAllMenus() {
        MenuService proxyInstance = (MenuService) Proxy.newProxyInstance(
    ...
    public List<Menu> searchMenu(MenuSearchCommand command) {
        ...
        // 이름 검색
        if (StringUtils.hasText(command.getName())) menus.removeIf(menu -> !menu.getName().contains(command.getName()));
        // 코드 검색
        if (StringUtils.hasText(command.getCode())) menus.removeIf(menu -> !menu.getCode().equals(command.getCode()));
        // 권한 명 검색
        if (StringUtils.hasText(command.getRoleName())) menus.removeIf(menu -> !menu.getRole().getName().equals(command.getRoleName()));
