...
public class RoleRestService implements RoleService {
    ...
    private RoleService createProxy() {
        return (RoleService) Proxy.newProxyInstance(
    ...
    public List<Role> searchAllByCondition(RoleSearchCommand command) {
        ...
        if(StringUtils.hasText(command.getName())) roles.removeIf(role -> !role.getName().contains(command.getName()));

        if(StringUtils.hasText(command.getCode())) roles.removeIf(role -> !role.getRoleType().getCode().contains(command.getCode()));

        if(!ObjectUtils.isEmpty(command.getName())) roles.removeIf(role -> role.getRank() != command.getRank());
    ...
    public Role findOneByCode(String code) {
        return getAllRoles().stream().filter(role -> role.getRoleType().getCode().equals(code)).findFirst().orElse(null);
    }
