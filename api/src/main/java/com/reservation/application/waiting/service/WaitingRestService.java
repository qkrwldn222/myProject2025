...
public class WaitingRestService implements WaitingService {
  ...
  public void leaveWaitingQueueByOwner(Long restaurantId) {
    ...
    if (!ownerUser.getId().equals(restaurant.getOwnerId()))
      throw new ApiException("가게 운영자가 아닙니다.", HttpStatus.BAD_REQUEST);
    ...
    for (String userId : subscribedUsers) {
      if (!waitingUsers.contains(userId)) {
        ...
        sseEmitterService.sendEvent(
            userId, "ESTIMATED_POSITION : " + (newJoinPosition + 1), restaurantId);
      }
    }
  }
}
