...
public class WaitingServiceTest {
  ...
  void setUp() {
    ...
    // Mock User Data
    testOwner = User.builder().userID(OWNER).username("Owner User").password("password").build();

    testUser1 = User.builder().userID(USER_1).username("Test User 1").password("password").build();

    testUser2 = User.builder().userID(USER_2).username("Test User 2").password("password").build();
    ...
    Mockito.when(restaurantService.findById(TEST_RESTAURANT_ID))
        .thenReturn(Optional.of(testRestaurant));
    //        Mockito.when(sseEmitterService.createEmitterKey(USER_1 ,
    // TEST_RESTAURANT_ID)).thenReturn(testUser1.getUserID()+":"+ TEST_RESTAURANT_ID);

  }

  /** GIVEN: 사용자가 웨이팅 대기열에 참여 WHEN: joinWaitingQueue 호출 THEN: 대기열에 사용자 추가됨 */
  @Test
  ...
  void testJoinWaitingQueue() {
    ...
    Awaitility.await()
        ...
        .untilAsserted(
            () -> {
              Mockito.verify(sseEmitterService)
                  .sendEvent(USER_1, "POSITION : 1", TEST_RESTAURANT_ID);
            });
  }

  /** GIVEN: 사용자가 대기열에서 나감 WHEN: leaveWaitingQueue 호출 THEN: 대기열에서 사용자 제거됨 */
  @Test
  @WithUserDetails(USER_1)
  ...
  /** GIVEN: 운영자가 다음 사용자 입장 요청 WHEN: allowNextUser 호출 THEN: 대기열 첫 번째 사용자에게 "ENTER" 알림 전송 */
  @Test
  ...
  /** GIVEN: 운영자가 대기열에서 실제 입장 확정 WHEN: confirmEntry 호출 THEN: 대기열 첫 번째 사용자가 제거됨 */
  @Test
  @WithUserDetails(OWNER)
  void testConfirmEntry() {
    ...
    //        Mockito.doNothing().when(sseEmitterService).sendEvent(Mockito.anyString(),
  ...
  /** GIVEN: 대기열에 사용자가 존재 WHEN: getWaitingList 호출 THEN: 대기 중인 사용자 ID 목록 반환 */
  @Test
  ...
  /** GIVEN: 특정 사용자가 대기 중 WHEN: getUserWaitingPosition 호출 THEN: 현재 대기 순서 반환 */
  @Test
